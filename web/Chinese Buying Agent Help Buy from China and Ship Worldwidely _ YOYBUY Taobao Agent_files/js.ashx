...
$(function () {
    ...
    //抓取url
    $("#sumbiturl").live("click", function () {
                var urlOne = $("#taobaourl").val();
                if (/^http(s)?:\/\/([\w-]+\.)+[\w-]+(\/[\w\- ./?%&=]*)?/.test(urlOne) &&!/[()<>]+/.test(urlOne)) {
                    $("#AddUrl").find("[name=url]").val(urlOne);
                    $("#AddUrl").submit();
                } else {
                    if (urlOne=="") {
                        window.location.href = "http://www.yoybuy.com/en/addurl.html";
                        return true;
                    }
                    MessageBox.showAlertMessageBoxWarn(630, 260, "* Error website address, please check it!", "OK", "");
                    return false;
                }
    ...
    //店铺过渡页
    redirectStoreUrl = function (url, storeName) {
        $("#storeUrl").val("");
        $("#storeName").val("");
        $("#storeUrl").val(url);
        $("#storeName").val(storeName);
        $("#storeFormSubmit").submit();
    }
...
// 0 为购物车有商品 1、 购物车没有商品
function ShowCartWin(num) {
    showDivId = num;
    $(".shoplist").hide();
    $(".shoplist:eq(" + num + ")").show();
}


function CloseBtn() {
    $("#closeHeadImg").click(function () { $("#HeadImgDiv").remove(); $("#header > div:first").addClass("homebor"); });
};
var UserId = "";
var cartNum = 0;
...
function SignOut() {
    $.ajax({
        ...
        success: function (data) {
            if (data.result == true) {
                $("#username").html("<a href='http://login.yoybuy.com/en/login' rel='nofollow'>Sign In</a>");
                $("#regOrSignOut").html("<a href='http://login.yoybuy.com/en/Register' rel='nofollow' class='floR noline norcol height30 marl10' >Register</a>");
                $("#facLogin").show();
...
var head = {
    GetUserName: function () { //获取用户名字
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.result == true) { //用户已经登陆
                    
                    $("#username").html("Welcome, <a class=\"norcol\" href=\"http://customercenter.yoybuy.com/en/userindex.html\">" + uname + "</a>");
                    $("#regOrSignOut").html("<div><a class='floR noline gray height30 marl10' href='javascript:void(0)' onclick='SignOut()' >Sign Out</a><div> <img src=\"http://img.yoybuy.com/V6/Common/topbg.png\" width=\"1\" height=\"14\" class=\"floR marl10 mar8\" />");
                    $("#facLogin").hide();

                } else {
                    $("#username").html('Welcome, ' + uname + '</a>');
                    $("#regOrSignOut").html("<div class=\"floR marl10 horegister\"><a href=\"http://login.yoybuy.com/en/Register\" class=\"noline norcol height30\">Register</a> <p class=\"clear ove\">$10 for new register</p>  </div><img src=\"http://img.yoybuy.com/V6/Common/topbg.png\" width=\"1\" height=\"14\" class=\"floR marl10 mar8\" /><div><a class='floR noline norcol  height30 marl10' href='http://login.yoybuy.com/en/login' >Sign In</a><div>");
                }
    ...
    GetCartNum: function () {//获取购物车数量
        $.ajax({
            ...
            success: function (data) {
                if (data.result == true) {
                    cartNum = data.data;
                    $("#goodsNum").html("(" + data.data + ")");
                }
            }
        });
    },
    GetTotalMoney: function () { //获取购物车商品总价值
        $.ajax({
            url: "http://shoppingcart.yoybuy.com/en/shoppingcart/GetCartTotalMoney",
            dataType: "jsonp",
            cache: false,
            success: function (data) {
                if (data.result == true) {
    ...
    GetCartList: function () {
        ...
        $("#ShopingCart").show();
        $.ajax({
            url: 'http://shoppingcart.yoybuy.com/shoppingcart/GetGoods',
            dataType: "jsonp",
            cache: false,
            success: function (data) {
                if (data.result) {
                    ...
                    for (var i = 0; i < data.data.length; i++) {
                        if (i == 3) { continue; }; //最多显示三条购物车记录
                        htmlvalue += "<div class='clear ove shopcone'><a href='" + data.data[i].Url + "' target='_blank' class='flo product50'><img src='" + data.data[i].Photo + "' width='50' height='50' /></a><dl class='floR'><dt class='clear ove'><a href='" + data.data[i].Url + "' target='_blank' class='norcol' title='" + data.data[i].Name + "'>" + data.data[i].Name + "</a></dt><dd class='clear ove mar2'><em class='flo'>US<span class='orangetip'>$" + data.data[i].Price + "</span> X " + data.data[i].Amount + "</em><a class='floR centers norcol noline' onclick=\"DeleteGood(" + data.data[i].GoodsId + ",this)\" ><div class='mailtip'><img src='http://img.yoybuy.com/V6/Common/newdelete.png' width='14' height='15' class='flo mar5 marr4' /><span class='flo'>Delete</span></div></a></dd></dl></div>";
                        ShowCartWin(0);
                    }
                    if (data.data.length > 3) { $("#ShopingCart").next().show(); }
    ...
    CartMover: function () {
        $(".carthover").mouseleave(function () {
            ...
            if ($("#ShopingCart").html().indexOf("loading.gif") > 0) {
                head.GetCartList();
            }
            $("#ShopingCart").show();
            $(".shoplist:eq(" + showDivId + ")").show();
        });
...
function getseverTime() {
    $.ajax({
        url: "http://www.yoybuy.com/en/home/GetSeverTime",
        type: 'POST',
        dataType: "jsonp",
        cache: false,
        success: function (data) {
            if (data.bjtime != null) {
                usaTime = new Date(data.bjtime);
                var timer = setInterval(function () {
                    usaTime.setSeconds(usaTime.getSeconds() + 1);
                    ShowTime();
                }, 1000);
            }
        }
...
function ShowTime() {
    var year = usaTime.getFullYear();

    var month = usaTime.getMonth() + 1;

    if (month < 10) {
        month = "0" + month;
    }

    var day = usaTime.getDate();
    ...
    $("#spanDate1").html(day + "/" + month + "/" + year + "");
    $("#spantime1").html("<em>" + hours + "</em>:<em>" + minutes + "</em>:<em>" + seconds + "</em>");
...
(function ($) {
    $.fn.extend({
        imageScrollViewer: function (images, links, curImgIndex, intervalSecond) {
            var imgArray = new Array();
            var currentImgIndex = curImgIndex;
            if (!curImgIndex) {
                currentImgIndex = 0;
            }
            ...
            var pImg = $("p[class='banimg']", $(this)).eq(0);
            var btnLeft = $("a[class='clear ove indexturn']", $(this)).eq(0);
            var btnRight = $("a[class='clear ove indexturnR']", $(this)).eq(0);
            btnLeft.hide();
            btnRight.hide();
            ...
            var controlBtn = function () {
                ...
                $("a[data-img-index]").removeClass("current");
                $("a[data-img-index='" + currentImgIndex + "']").addClass("current");
                pImg.css("background-image", "url('" + imgArray[currentImgIndex] + "')");
                $("p[class='banimg']").unbind("click").bind("click", function () {
                    window.open(links[currentImgIndex]);
                });
                $("p[class='banimg']").css("cursor", "pointer");
            };
            ...
            var indexNumContianer = $("<div class=\"clear ove indexnum centers\"></div>").appendTo(imgContainer);
            var numContainer = $("<div class=\"clear ove mailtip\"></div>").appendTo(indexNumContianer);
            for (var i = 0; i < imgCount; i++) {
                var className = "flo marr25";
                if (i.toString() === currentImgIndex.toString()) {
                    className = "flo marr25 current";
                }
                else if (i === imgCount - 1) {
                    className = "flo";
                }
                numContainer.append("<a class=\"" + className + "\" data-img-index=\"" + i + "\"></a>");
            }

            $("a[data-img-index]").unbind("click").bind("click", function () {
                currentImgIndex = $(this).attr("data-img-index");
                controlBtn();
            });
...
YOYBUYMonitor.go = function () {
    var cookie = {};
    var scriptArray = document.getElementsByTagName('script'), ss_domain = "http://resources.yoybuy.com";
    var isExist = true;
    var data = {};
    data.sid = "";
